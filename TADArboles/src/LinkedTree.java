...
public class LinkedTree<E> extends DrawableTree<E> {
    ...
    private class TreeNode<T> implements Position<T> {
        ...
        private void setElement(E temp) {
    ...
    private void breadthFirstTraversal(TreeNode<E> root, List<Position<E>> positions) {
        if(root!=null){
            List<TreeNode<E>> cola = new ArrayList<>();
            cola.add(root);
            while(!cola.isEmpty()){
                TreeNode<E> colavisit = cola.get(0);
                positions.add(colavisit);
                cola.addAll(colavisit.getChildren());
            }
